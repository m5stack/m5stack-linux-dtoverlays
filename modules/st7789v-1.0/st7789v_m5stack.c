...
#define DRVNAME "fb_st7789v_m5stack"
...
static u32 ram_x_offset = 0;
static u32 ram_y_offset = 0;
...
static int init_display(struct fbtft_par *par)
{
    ...
    /* Read ram-x-offset from device tree */
    rc = device_property_read_u32(dev, "ram-x-offset", &val);
    if (!rc) {
        ram_x_offset = val;
        dev_info(dev, "ram-x-offset: %u\n", ram_x_offset);
    } else {
        ram_x_offset = 0;
    }
    
    /* Read ram-y-offset from device tree */
    rc = device_property_read_u32(dev, "ram-y-offset", &val);
    if (!rc) {
        ram_y_offset = val;
        dev_info(dev, "ram-y-offset: %u\n", ram_y_offset);
    } else {
        ram_y_offset = 0;
    }
...
static int set_var(struct fbtft_par *par)
{
	...
	case 270:
		...
		x_offset = row_offset;		y_offset = col_offset;
		break;
	default:
		return -EINVAL;
	}
...
FBTFT_COMPATIBLE("sitronix,st7789v_m5stack")
FBTFT_VARIANT_COMPATIBLE("fbtft,minipitft13", variant_minipitft13)
FBTFT_REGISTER_DRIVER_END(DRVNAME, &display);

MODULE_ALIAS("spi:" DRVNAME);
MODULE_ALIAS("platform:" DRVNAME);
MODULE_ALIAS("spi:st7789v_m5stack");
MODULE_ALIAS("platform:st7789v_m5stack");

MODULE_DESCRIPTION("FB driver for the st7789v_m5stack LCD Controller");
MODULE_AUTHOR("Dennis Menschel, dianjixz");
MODULE_LICENSE("GPL");
